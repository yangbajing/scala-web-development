...
class FusionCore private (system: ActorSystem[_])
    extends Extension
    with StrictLogging {
  ...
  def spawnActor[REF](behavior: Behavior[REF], name: String)(
      implicit timeout: Timeout): Future[ActorRef[REF]] =
  ...
  def receptionistFindOne[T](
      ...
      timeout: FiniteDuration): ActorRef[T] = {
  ...
  def receptionistFindSet[T](
      serviceKey: ServiceKey[T],
      timeout: FiniteDuration): Set[ActorRef[T]] = {
    implicit val t: Timeout = Timeout(timeout)
    val f =
      system.receptionist
...
object FusionCore extends ExtensionId[FusionCore] {
  override def createExtension(system: ActorSystem[_]): FusionCore =
    new FusionCore(system)
}
