...
object AkkaUtils extends StrictLogging {
  def findActorByServiceKey[T](
      ...
      implicit system: ActorSystem[_]): Future[Option[ActorRef[T]]] = {
    ...
    def _find(count: Int): Future[Option[ActorRef[T]]] = {
      val f = findActorByServiceKey(serviceKey).recover {
      ...
      f.flatMap {
        ...
        case None =>
          logger.error(
            s"findActorByServiceKey error：$count/$total ($serviceKey, $timeout, $interval).")
          Future.successful(None)
        case some =>
          logger.debug(
            s"findActorByServiceKey find：$count/$total ($serviceKey, $timeout, $interval).")
  ...
  private def findActorByServiceKey[T](serviceKey: ServiceKey[T])(
      implicit system: ActorSystem[_],
      timeout: Timeout): Future[Option[ActorRef[T]]] = {
    system.receptionist
