...
object Configs {
  ...
  case class UpdateContent(
      ...
      extends Command
  case class RemoveContent(dataId: String, replyTo: ActorRef[ModifyReply])
      extends Command
  case class AllContent(replyTo: ActorRef[immutable.Seq[ConfigContent]])
      extends Command

  case class RegisterListener(
      listenerId: UUID,
      dataId: String,
  ...
  private case class InternalSubscribeResponse(
      chg: Replicator.SubscribeResponse[ReplicatorType])
  ...
  def apply(namespaceKey: NamespaceKey): Behavior[Command] = Behaviors.setup {
    context =>
      implicit val node: SelfUniqueAddress =
        DistributedData(context.system).selfUniqueAddress

      DistributedData.withReplicatorMessageAdapter[Command, ReplicatorType] {
        replicatorAdapter =>
          replicatorAdapter.subscribe(namespaceKey, InternalSubscribeResponse.apply)

          def active(
              listeners: List[RegisterListener],
              cachedConfigs: Map[String, ConfigContent]): Behavior[Command] = {
            Behaviors.receiveMessagePartial[Command] {
              case GetContent(dataIds, replyTo) =>
                val isCachedExists = cachedConfigs.nonEmpty && cachedConfigs.keysIterator
                    .forall(key => dataIds.contains(key))
                ...
                if (isCachedExists) {
                  ...
                  replicatorAdapter.askGet(
                    Replicator
                      .Get(namespaceKey, Replicator.ReadMajority(READ_TIMEOUT)),
              ...
              case UpdateContent(dataId, content, replyTo) =>
                replicatorAdapter.askUpdate(
                  Replicator.Update(
                    ...
                    Replicator.WriteAll(WRITE_TIMEOUT)) { orMap =>
                    orMap :+ (dataId -> ConfigContent(
              ...
              case RemoveListener(listenerId) =>
                active(
                  listeners.filterNot(_.listenerId == listenerId),
                  cachedConfigs)
            }
          }
          ...
          def receiveInternalCommand(
            ...
            internal match {
              ...
              case InternalUpdateResponse(chg, replyTo) =>
                chg match {
                  case Replicator.UpdateSuccess(`namespaceKey`) =>
                  ...
                  case _ =>
                    replyTo ! ModifyReply(IntStatus.INTERNAL_ERROR, "Error")
                }
                Behaviors.same
              ...
              case InternalRemoveResponse(chg, replyTo) =>
                chg match {
                  ...
                  case other =>
                    context.log.error(
                      s"InternalRemoveResponse($chg, $replyTo) error: $other")
                  ...
                  case v @ Replicator.Changed(`namespaceKey`) =>
                    ...
                    entries.foreach {
                      case (_, cc) =>
                        ...
                        val changeType =
                          if (isChange) 2 else if (maybeOld.isEmpty) 1 else 0
                        if (changeType != 0) {
                          val modified = ConfigChanged(
                            ...
                            cc.content,
                            oldContent =
                              cachedConfigs.get(cc.namespace).map(_.content),
          ...
          def notifyListeners(
              listeners: List[RegisterListener],
              cc: ConfigContent,
              changed: ConfigChanged): Unit = {
            for (listener <- listeners if listener.dataId == cc.namespace) {
              listener.queue
                .offer(changed)
                .foreach {
                  case QueueClosed =>
                    context.self ! RemoveListener(listener.listenerId)
                  case _ => // do nothing
